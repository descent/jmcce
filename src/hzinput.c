...
unsigned char *half_full_strBIG5[] = { "【半形】", "【全形】" };
unsigned char *half_full_strGB[] = { "▽圉褒▼", "▽�娃ョ�" };
unsigned char *history_normal_strBIG5[] = { "【輸入】", "【歷史】" };
unsigned char *history_normal_strGB[] = { "▽怀�諢�", "▽盪妢▼" };
...
BarRight (int xmax, int ymax, int leftmar, int stepwidth)
{
  ...
  if (mx == xmax && my == ymax) {
	...
	if (gEncode == GB) {
	  strcat (Item_str[jj][ii],
		  ((jj + gItem_disp_off) ==
		   10) ? string_BIG5toGB ("內碼") : (unsigned char
